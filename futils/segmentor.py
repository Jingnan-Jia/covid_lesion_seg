...
import numpy as np
from jjnutils.vpatch import deconstruct_patch_gen, reconstruct_patch_gen
...
class v_segmentor(object):
    def __init__(self, batch_size=1, model='.hdf5', ptch_sz=128, ptch_z_sz=64, trgt_sz=None, trgt_z_sz=None,
        ...
        elif task in ['vessel', 'lesion']:
            self.labels = [0, 1]
        else:
            raise Exception('please assign the task name for prediction', task)
    ...
    def predict(self, x, ori_space_list=None, stride=0.25):
        ...
        if any(self.trgt_space_list) or (self.trgt_sz_list is not None and any(self.trgt_sz_list)):
            x = cu.downsample(x_ori, is_mask=False,
        ...
        if (self.task in ["lobe"]) or (a2 is None):  # deconstruct low resolution ct, high resolution ct as aux input
        ...
        pred = reconstruct_patch_gen(pre_gen, ptch_shape=patch_shape, original_shape=x.shape, stride=stride, chn=chn,
                                     mot=self.mot, original_shape2=original_shape2, task=self.task)  # todo: how to compute fissure, metrics for 2 output ct?
        # one hot decoding
        masks = []
        if type(pred) is list:
            pred = pred[0]
        for p in pred:
            masks.append(cu.one_hot_decoding(p, self.labels))
