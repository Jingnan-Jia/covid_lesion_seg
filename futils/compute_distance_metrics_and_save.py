...
def write_all_metrics_for_one_ct(mylock, labels, gdth_name, pred_name, csv_file, lung, fissure):
    ...
    if lung:  # gdth is lung, so pred need to convert to lung from lobes, labels need to be [1],
        ...
        if not gdth.shape == pred.shape:  # sometimes the gdth size is different with preds.
            pred = cu.downsample(pred, ori_sz=pred.shape, trgt_sz=gdth.shape, order=1,
                              labels=labels, train=False)  # use shape to upsampling because the space is errors sometimes in LOLA11
        ...
        pred = pred_cp
        cu.save_itk(pred_name.split(".mh")[0]+"_points.mha", pred, pred_origin, pred_spacing)
...
def write_all_metrics(labels, gdth_path, pred_path, csv_file, fissure=False, fissureradius=1, lung=False, workers=1):
    ...
    gdth_names, pred_names = cu.get_gdth_pred_names(gdth_path, pred_path, fissure=fissure, fissureradius=fissureradius)
